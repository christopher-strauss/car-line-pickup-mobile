...
const MyFamilyScreen = ({navigation}) => {
  ...
  const [familyMembers, setData] = useState([]);

  const { response, loading, error } = useAxios({
      method: 'get',
      url: '/api/v1.0/families/16B8147F-B7EC-4836-9488-9BDC4351CAFC/familymembers?pageSize=20&pageIndex=0&orderBy=familyMemberType desc',
    });
      ...
      <VStack h="100%">
        <Stack p="4" space={1} />
        {
          loading ? (
            ...
            <Box>
              <FlatList data={familyMembers.data} renderItem={({ item }) => 
                <Box alignItems="center">                
                      ...
                      <Center bg="violet.500" _dark={{
                        bg: "violet.400"
                          ...
                          fontSize: "xs"
                            ...
                            {item.familyMemberType.toUpperCase()}
                      </Center>
                    </Box>
                    <Stack p="4" space={1}>
        ...
        <TouchableOpacity style={styles.plusbutton} 
                onPress={() => {
                  navigation.navigate('FamilyMemberScreen', {
                    id: ''
                  });
                }}>
